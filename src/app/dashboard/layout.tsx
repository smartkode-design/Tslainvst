...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-[100dvh] bg-slate-50 overflow-hidden selection:bg-primary/30">
      
      {/* Sidebar - Desktop Only (Crisp Solid Fintech Architecture) */}
      <aside className="hidden md:flex w-72 flex-col border-r border-slate-200/80 bg-white relative z-20 shadow-xs">
        ...
        <div className="flex-1 overflow-auto py-6 px-4 flex flex-col gap-1.5 z-10">
          ...
          <div className="mt-8 mb-2 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
            Preferences & Help
          </div>
          ...
          {bottomNavItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="flex items-center gap-3.5 px-4 py-3 rounded-2xl text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all duration-200 group"
            >
              <item.icon className="h-5 w-5 group-hover:scale-105 transition-transform duration-200 text-slate-400 group-hover:text-slate-600" />
              {item.name}
            </Link>
          ))}
        </div>
        ...
        <div className="p-4 border-t border-slate-100 z-10">
          <Link href="/settings">
            <div className="flex items-center gap-3.5 p-3 rounded-2xl bg-slate-50 border border-slate-200/80 hover:bg-slate-100/80 transition-colors cursor-pointer shadow-2xs">
              ...
              <div className="flex-1 overflow-hidden">
                <p className="text-sm font-bold text-slate-900 truncate">Oluwaseun A.</p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <header className="md:hidden h-16 flex items-center justify-between px-4 z-30 pt-1 border-b border-slate-200/60 bg-white/80 backdrop-blur-md">
          ...
          <Link href="/support">
            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-slate-700 bg-slate-100 hover:bg-slate-200 relative">
        ...
        {/* Desktop Header */}
        <header className="hidden md:flex h-20 items-center justify-between px-10 z-10 border-b border-slate-200/60 bg-white/50 backdrop-blur-md">
          <div className="flex-1 flex items-center max-w-xl">
            <div className="relative w-full group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within:text-primary transition-colors" />
              <Input 
                placeholder="Search products, services, transactions..." 
                className="pl-11 bg-white border-slate-200 hover:border-slate-300 focus-visible:border-primary focus-visible:ring-primary/20 h-12 rounded-2xl text-sm font-semibold text-slate-900 placeholder:text-slate-400 shadow-2xs transition-all" 
              />
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Link href="/support">
              <Button variant="outline" className="h-11 px-4 rounded-xl bg-white border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs shadow-2xs flex items-center gap-2">
                <HelpCircle className="h-4 w-4 text-slate-400" />
                Support
              </Button>
            </Link>
            <Button variant="outline" size="icon" className="h-11 w-11 rounded-xl bg-white border-slate-200 text-slate-700 hover:bg-slate-50 relative shadow-2xs">
              <Bell className="h-4 w-4" />
              <span className="absolute top-2.5 right-2.5 h-2 w-2 rounded-full bg-primary border-2 border-white animate-pulse"></span>
            </Button>
          </div>
        </header>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-18 bg-white/95 backdrop-blur-xl border-t border-slate-200 z-40 pb-1 shadow-[0_-4px_25px_rgba(0,0,0,0.04)] flex items-center justify-around px-2">
          {mobileTabs.map((tab) => {
