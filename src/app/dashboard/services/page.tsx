...
export default function ServicesHubPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Services Hub</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {cat.services.map((srv, srvIdx) => (
                <Link key={srvIdx} href={srv.href}>
                  <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs hover:shadow-lg hover:border-primary/40 transition-all duration-300 flex items-start gap-4 group cursor-pointer h-full">
                    <div className={`h-12 w-12 rounded-2xl bg-gradient-to-br ${srv.color} p-[1px] shadow-sm shrink-0 group-hover:scale-105 transition-transform ${srv.shadow}`}>
                      <div className="w-full h-full bg-white/20 rounded-[0.95rem] flex items-center justify-center backdrop-blur-md">
