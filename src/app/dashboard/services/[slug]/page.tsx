...
export default function ServicePage({ params }: { params: { slug: string } }) {
  ...
  const smmServices = [
    {
      ...
      category: "followers",
      quality: "HIGH",
      qualityColor: "bg-blue-50 text-blue-700 border-blue-200",
    ...
    },
    {
      ...
      category: "followers",
      quality: "FARM",
      qualityColor: "bg-purple-50 text-purple-700 border-purple-200",
    ...
    },
    {
      ...
      category: "likes",
      quality: "HIGH",
      qualityColor: "bg-emerald-50 text-emerald-700 border-emerald-200",
    ...
    },
    {
      ...
      category: "views",
      quality: "LOW",
      qualityColor: "bg-amber-50 text-amber-700 border-amber-200",
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Virtual Numbers (SMS OTP)</h1>
              <p className="text-xs text-slate-500 font-medium">Rent non-VoIP private numbers for instant 2FA SMS verification codes</p>
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full text-xs font-bold text-emerald-700">
        ...
        {/* Balance Card */}
        <div className="bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-3xl p-5 sm:p-6 shadow-md flex items-center justify-between">
          <div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-slate-400">8 Supported Apps</span>
          </div>
            ...
            {otpServices.map((srv) => (
              ...
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-bold text-slate-900 leading-tight truncate">{srv.name}</span>
                  <span className="text-[10px] font-extrabold text-primary mt-0.5">{srv.startingPrice}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-emerald-600">All Non-VoIP</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {otpCountries.map((c) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-6 rounded border border-slate-200 overflow-hidden shadow-xs shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-semibold text-slate-400">{c.code} · {c.stock}</p>
                  </div>
                </div>
                <span className="text-sm font-black text-slate-900">₦{c.price.toLocaleString()}</span>
              </div>
            ))}
          </div>
          ...
          <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-xs text-slate-500 font-medium">
              Order: <strong className="text-slate-900">{activeCountryObj.name} ({activeCountryObj.code})</strong> for <strong className="text-slate-900">{activeServiceObj.name}</strong> · Rate: <strong className="text-primary">₦{activeCountryObj.price.toLocaleString()}</strong>
            </div>
            <Button
              onClick={() => {
        ...
        {hasGeneratedNumber && (
          <div className="bg-white rounded-3xl border-2 border-primary p-6 sm:p-8 shadow-xl shadow-primary/10 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Top Status Bar */}
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400">Direct carrier routing assigned</p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 text-xs font-black text-amber-700 bg-amber-50 border border-amber-200/80 px-3 py-1.5 rounded-xl">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <Button 
                ...
                className="h-11 px-6 rounded-xl bg-slate-900 text-white hover:bg-primary font-bold text-xs flex items-center gap-2 shadow-sm"
              >
            ...
            {!smsReceived ? (
              <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 bg-slate-50/50 flex flex-col items-center justify-center text-center space-y-3">
                <div className="h-14 w-14 rounded-full bg-primary/10 flex items-center justify-center text-primary animate-pulse">
                  <RefreshCw className="h-7 w-7 animate-spin duration-1000" />
                </div>
                <div>
                  <h4 className="font-extrabold text-slate-900 text-base">Listening for incoming SMS...</h4>
              ...
              <div className="border-2 border-emerald-500/40 rounded-2xl p-6 bg-emerald-50/40 space-y-4 animate-in zoom-in-95 duration-300">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <BadgeCheck className="h-5 w-5 text-emerald-600" />
                    <span className="text-xs font-black uppercase text-emerald-900 tracking-wider">SMS Received Successfully!</span>
                  </div>
                  <span className="text-[10px] font-bold text-emerald-700 bg-emerald-100 px-2.5 py-0.5 rounded-full">Just now</span>
                </div>

                <div className="bg-white border border-emerald-200 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
                  <div className="space-y-0.5 text-center sm:text-left">
                    <p className="text-xs font-semibold text-slate-500">Your {activeServiceObj.name} verification code is:</p>
                    <p className="text-3xl font-black text-slate-900 font-mono tracking-widest">849 - 204</p>
                  </div>
                  <Button 
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Boost Services (SMM Panel)</h1>
              <p className="text-xs text-slate-500 font-medium">Grow your social media presence with instant guaranteed engagements</p>
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-1.5 bg-primary/10 text-primary border border-primary/20 px-3 py-1 rounded-full text-xs font-bold">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-3.5 shadow-xs flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 font-bold text-slate-700">
            <ShieldCheck className="h-4 w-4 text-primary" />
            <span>Service Quality Guide:</span>
          </div>
          <div className="flex flex-wrap items-center gap-2 font-bold text-[11px]">
            <span className="flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-md">🟡 LOW</span>
            <span className="flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-md">🟢 MEDIUM</span>
            <span className="flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md">🔵 HIGH</span>
            <span className="flex items-center gap-1 bg-purple-50 text-purple-700 border border-purple-200 px-2 py-0.5 rounded-md">🟣 FARM</span>
            <span className="flex items-center gap-1 bg-rose-50 text-rose-700 border border-rose-200 px-2 py-0.5 rounded-md">🟠 PROVIDER</span>
          </div>
        </div>
        ...
        {/* Step 2: Specific Service Selection List (Paxplug Style) */}
        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-4">
          <label className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-2">
            <span className="h-5 w-5 rounded-full bg-primary text-white text-[10px] flex items-center justify-center font-bold">2</span>
              ...
              .map((srv) => (
                ...
                >
                  ...
                  <div className="text-left sm:text-right shrink-0">
                    <span className="text-base font-black text-slate-900">₦{srv.rate.toLocaleString()}</span>
                    <span className="text-xs text-slate-400 font-bold block">per 1,000</span>
                  </div>
                </div>
              ))}
          </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                className="h-13 rounded-2xl bg-slate-50 border-slate-200 font-semibold text-sm text-slate-900"
              />
              <p className="text-[10px] text-slate-400 font-medium">Make sure the account is set to public, not private.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                placeholder="1000" 
                className="h-13 rounded-2xl bg-slate-50 border-slate-200 font-black text-base text-slate-900"
              />
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/70 flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm font-black text-slate-900">₦248,500.00</p>
            </div>
            <div className="text-right space-y-0.5">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Total Charge</span>
              <p className="text-xl font-black text-primary">₦{Math.round(totalPrice).toLocaleString()}</p>
            </div>
          </div>

          {smmSuccess ? (
            <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-center font-bold text-sm flex items-center justify-center gap-2">
      ...
      <div className="max-w-2xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Buy Data</h1>
              <p className="text-xs text-slate-500 font-medium">Instant 24/7 automated data bundle delivery</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-6">
          <div className="space-y-2.5">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Select Network Provider</label>
            <div className="grid grid-cols-4 gap-2.5">
              {networks.map((net) => (
                ...
                >
                  ...
                  <span>{net.name}</span>
                  <span className={`text-[9px] font-semibold opacity-80 ${selectedNetwork === net.id ? "text-white" : "text-slate-400"}`}>
                    {net.discount}
                  </span>
                </button>
              ))}
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Recipient Phone Number</label>
            <Input 
              value={phoneNumber}
              onChange={(e) => setPhoneNumber(e.target.value)}
              placeholder="e.g. 0812 345 6789" 
              className="h-13 rounded-2xl bg-slate-50 border-slate-200 font-bold text-base text-slate-900"
            />
          </div>

          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Select Data Plan</label>
                ...
                {["SME", "Gifting", "Corporate"].map((cat) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-[10px] font-black transition-all ${
                      dataCategory === cat.toLowerCase() ? "bg-white text-slate-900 shadow-xs" : "text-slate-500"
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
              {dataPlans.map((plan) => (
                ...
                >
                  ...
                  <span className="text-base font-black text-slate-900 mt-1">{plan.size}</span>
                  <span className="text-xs font-extrabold text-primary">{plan.price}</span>
                  <span className="text-[9px] font-bold text-slate-400">{plan.validity}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Buy Airtime</h1>
            <p className="text-xs text-slate-500 font-medium">Instant top-up with up to 3% discount</p>
          </div>
        </div>

        <div className="bg-white rounded-3xl border border-slate-200/80 p-5 sm:p-7 shadow-sm space-y-6">
          <div className="space-y-2.5">
            <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Select Network</label>
            <div className="grid grid-cols-4 gap-2.5">
              {networks.map((net) => (
                <button
                  key={net.id}
          ...
          <div className="space-y-2.5">
            ...
            <Input 
              ...
              type="number"
              placeholder="1000" 
              className="h-13 rounded-2xl bg-slate-50 border-slate-200 font-black text-lg text-slate-900"
            />
              ...
              {["100", "200", "500", "1000", "2000", "5000"].map((amt) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${
                    airtimeAmount === amt ? "bg-primary text-white border-primary" : "bg-slate-50 text-slate-600 border-slate-200"
                  }`}
                >
                  ₦{amt}
                </button>
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-4 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Affiliate Website</h1>
            <p className="text-xs text-slate-500 font-medium">Own your own automated digital platform connected to our APIs</p>
          </div>
        </div>

        <div className="bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-950 text-white rounded-3xl p-6 sm:p-8 relative overflow-hidden shadow-xl">
        ...
        <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-8">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                ...
                >
                  <span className="text-lg font-black text-slate-900">{domain.ext}</span>
                  <span className="text-base font-extrabold text-primary">{domain.price}</span>
                  <span className="text-[10px] font-bold text-slate-400">{domain.tag}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden focus-within:ring-2 focus-within:ring-primary/20">
              <span className="px-4 py-3.5 text-xs font-bold text-slate-400 border-r border-slate-200 bg-slate-100">www.</span>
              <Input 
                value={domainName}
                onChange={(e) => setDomainName(e.target.value)}
                placeholder="yoursite" 
                className="border-0 bg-transparent h-12 font-bold text-slate-900 focus-visible:ring-0"
              />
              <span className="px-4 py-3.5 text-xs font-black text-primary border-l border-slate-200 bg-slate-100">{selectedDomain}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input 
              ...
              className="h-12 rounded-2xl bg-slate-50 border-slate-200 font-semibold text-slate-900"
            />
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-24 md:pb-12 pt-2 md:pt-6 px-4">
      ...
      <Card className="bg-white border border-slate-200/80 rounded-3xl p-6">
        <p className="text-slate-500 text-sm font-medium">Service configuration ready.</p>
      </Card>
    </div>
  );
}
